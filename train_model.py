...
def train_hybrid(
    ...
    # Freeze RoBERTa to speed up CPU training
...
def main():
    ...
    if not args.baseline_only:
        # Optimization: Take a subset for fast re-fit on CPU
        train_subset = train_df.head(2000) 
        val_subset = val_df.head(500)
        
        hybrid_result = train_hybrid(
            train_subset, val_subset,
            epochs=args.epochs, lr=1e-4, # Higher LR for head
            batch_size=8, max_len=128,  # Smaller for CPU
        )
